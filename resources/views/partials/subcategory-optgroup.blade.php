...
if (isset($dataArray['item']['new_category_id'])) {
    $decodedIds = json_decode($dataArray['item']['new_category_id'], true);
...
if ($isArray) {
    $categoryIds = json_decode($dataArray['item']->new_category_id);
    $selected = in_array($sub_category_id, $categoryIds) ? "selected" : "";
} else {
