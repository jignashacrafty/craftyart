            ...
            <div class="d-flex">
                <select id="email_template_id" class="form-control email_template_id">
                    ...
                    @foreach ($emailTemplates as $tpl)
                        <option value="{{ $tpl->id }}"
                            {{ old('template', $configs['email_checkout_drop_automation']['template'] ?? '') == $tpl->id ? 'selected' : '' }}>
                            {{ $tpl->id }} - {{ $tpl->name }}
                        </option>
                    @endforeach
                </select>
                ...
                <select name="promo_code" id="main_promo_code" class="form-control promo_code">
                    ...
                    @foreach ($promoCodes as $promoCode)
                        <option value="{{ $promoCode->id }}"
                            {{ old('promo_code', $configs['email_checkout_drop_automation']['promo_code'] ?? '') == $promoCode->id ? 'selected' : '' }}>
