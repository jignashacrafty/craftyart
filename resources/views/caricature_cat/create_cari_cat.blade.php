   ...
   <script>
       $('#dynamic_form').on('submit', function(event) {
           ...
           let longDesc = document.querySelector('textarea[name="long_desc"]').value;
           // if(!contents && !longDesc){
           //   window.alert("Please add Long desc or Contents")
           //   return
           // }
           count = 0;
           $.ajaxSetup({
               ...
               success: function(data) {
                   ...
                   if (data.error) {
                       ...
                       $('#result').html('<div class="alert alert-success">' + data.success +
                           '</div>');
                       window.location.href = "{{ route('show_cari_cat') }}";
                   }
                   setTimeout(function() {
                       $('#result').html('');
