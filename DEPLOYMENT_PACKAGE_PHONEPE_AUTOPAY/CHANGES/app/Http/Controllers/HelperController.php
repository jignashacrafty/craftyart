...
class HelperController extends Controller
{
    ...
    public static string $mediaUrl = "http://192.168.29.64/";
    ...
    public static function getParentVideoCatName($id, $isFromVideoCategory = false)
    {
        $res = \App\Models\Video\VideoCat::with('parentCategory')->where('id', $id)->first();
        if ($res) {
            if ($isFromVideoCategory && $id != 0) {
                return $res->category_name;
            }
            if (isset($res->parentCategory->category_name)) {
                return $res->parentCategory->category_name;
