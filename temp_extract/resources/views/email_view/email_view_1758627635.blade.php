...
<head>
    ...
    <style>
        ...
        .header {
            background: linear-gradient(180deg, #5fd661, #FDFDFF);
            padding: 35px 20px;
            text-align: center;
        }
    ...
    <div class="container">
        ...
        @if ($data['type'] == 'template')
            @if (!empty($data['data']['amount']) && count($data['data']['templates']) > 1)
                <div class="content">
                    <a href="{{ $data['link'] ?? '#' }}" target="_blank" class="btn" style="margin: 0 !important;">Buy
                        Now</a>
                </div>
            @endif
        @endif
        ...
        <div class="content">
            @if (!empty($data['data']['amount']))
                <p class="offer">
                    Last chance! Your
                    <strong>{{ $data['data']['amount'] }}</strong> offer ends soon
                </p>
            @endif
            @if ($data['type'] == 'plan' && !empty($data['data']['actual_price']) && !empty($data['data']['price']))
                <p class="offer">
                    This is your final reminder to grab the Crafty Art Bundle for just
                    <strong>{{ $data['data']['price'] }}</strong> After this, the price goes back to
                    <strong>{{ $data['data']['actual_price'] }}</strong> Don’t miss out!&#128540;
                </p>
            @endif
            {{-- <p>
        ...
        <div class="footer">
            Still confused? Reply to this email or WhatsApp us at <a href="tel:+919898972007">+91 98989 78207</a>
        </div>
    </div>
</body>
