                        ...
                        @if (!$isFreePlan)
                            ...
                            <div class="row mt-2">
                                <div class="col-md-2 col-sm-12">
                                    <div class="form-group">
                                        ...
                                        <input class="form-control inr-price" min="0" name="plan_price[]"
                                            type="number" step="any" placeholder="Enter Discount">
                                    </div>
                                </div>
                                ...
                                <div class="col-md-3 col-sm-12">
                                    <div class="form-group">
                                        ...
                                        <input class="form-control inr-offer-price" min="0" type="number"
                                               name="plan_price[]" step="any" placeholder="Amount">
                                    </div>
                                </div>
                                ...
                                <div class="col-md-2 col-sm-12">
                                    <div class="form-group">
                                        ...
                                        <input class="form-control usd-price" type="number" min="0"
                                            name="plan_price[]" step="any" placeholder="Enter Discount">
                                    </div>
                                </div>
                                ...
                                <div class="col-md-2 col-sm-12  mb-3 d-flex align-items-end">
                                    <button type="button" class="btn btn-primary" onclick="addCondition()">Add to
                                        Table</button>
                                </div>
                            </div>
                                ...
                                <tbody>
                                    @if ($subPlan->isNotEmpty())
                                        @foreach ($subPlan as $sub)
                                            {{-- @dd($sub->category->duration) --}}
                                            @php $data = $sub->plan_details ?? []; @endphp
                                                        ...
                                                        <td colspan="6">
                                                            <div class="rounded"
                                                                ...
                                                                <div class="row align-items-center"
                                                                    ...
                                                                    <div class="col-md-3">
                                                                        <label class="form-label mb-1">Monthly /
                                                                            Daily</label>
                                                                        <select
                                                                            name="feature_meta_value[{{ $rowFeature->id }}][meta_feature_value][]"
...
<script>
    ...
    console.log("dsasdasasd",requiredSlug);
</script>
<script>
    let editingRow = null;
    ...
    function editRow(button) {
        ...
        document.querySelector('input[name="additional_plan[]"]').value = row.children[1].textContent.trim();
        document.querySelector('.inr-price').value = row.children[2].textContent.trim();
        document.querySelector('.inr-offer-price').value = row.children[3].textContent.trim();
        document.querySelector('.usd-price').value = row.children[4].textContent.trim();
        document.querySelector('.usd-offer-price').value = row.children[5].textContent.trim();
    ...
    function setAddButtonText(text) {
        document.querySelector('button[onclick="addCondition()"]').textContent = text;
    }
    ...
    $('#createPlanForm').on('submit', function(e) {
        ...
        $('#conditionsTable tbody tr').each(function(i, row) {
            formData.append(`subplans[${i}][duration_value]`, $(row).find('td').eq(1).text().trim());
            formData.append(`subplans[${i}][inr_price]`, $(row).find('td').eq(2).text().trim());
            formData.append(`subplans[${i}][inr_offer_price]`, $(row).find('td').eq(3).text().trim());
            formData.append(`subplans[${i}][usd_price]`, $(row).find('td').eq(4).text().trim());
            formData.append(`subplans[${i}][usd_offer_price]`, $(row).find('td').eq(5).text().trim());
            formData.append(`subplans[${i}][duration_id]`, $(row).find('td').eq(0).data('category-id'));
            formData.append(`subplans[${i}][string_id]`, $(row).find('td').eq(0).data('string-id'));
    ...
    $("#planName").on("input", function() {
        ...
        $("#planID").val(titleString.toLowerCase().replace(/\s+/g, '-'));
