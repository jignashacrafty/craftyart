            ...
            <div class="card-box mb-30">
                <div class="pb-20">
                    ...
                    <table class="data-table table stripe hover nowrap">
                        ...
                        <tbody>
                            @foreach ($resultData as $tempPurchase)
                                <tr>
                                    <td class="table-plus">{{ $tempPurchase->id }}</td>
                                    <td>{{ \App\Http\Controllers\HelperController::getTemplateName($tempPurchase->product_id) }}
                                    </td>
                                    <td>{{ $tempPurchase->amount }}</td>
                                    <td>{{ $tempPurchase->from_where }}</td>
                                    @if ($tempPurchase->status == '1')
                                        <td>Active</td>
                                    @else
                                        <td>Disabled</td>
                                    @endif
                                    <td>
                                        <Button class="dropdown-item"
                                            onclick="edit_click('{{ $tempPurchase->id }}', '{{ $tempPurchase->product_id }}','{{ $tempPurchase->transaction_id }}', '{{ $tempPurchase->product_type }}', '{{ $tempPurchase->currency_code }}', '{{ $tempPurchase->amount }}', '{{ $tempPurchase->payment_method }}','{{ $tempPurchase->from_where }}', '{{ $tempPurchase->isManual }}', '{{ $tempPurchase->status }}','{{ $user_id }}')"
                                            data-backdrop="static" data-toggle="modal"
                                            data-target="#edit_package_model"><i class="dw dw-edit2"></i> Edit</Button>
                                        <Button class="dropdown-item"
                                            onclick="delete_click('{{ $tempPurchase->id }}')"><i
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="myLargeModalLabel">Add User Template Purchase</h5>
...
<div class="modal fade" id="edit_package_model" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel"
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="myLargeModalLabel">Edit User Template Purchase</h5>
                <button type="button" class="close" data-bs-dismiss="modal" aria-hidden="true">×</button>
            </div>
...
<script>
    ...
    $('#add_package_form').on('submit', function(event) {
        ...
        $.ajax({
            url: "{{ route('manage_template_product.submit') }}",
            type: 'POST',
            data: formData,
            beforeSend: function() {
    ...
    $(document).on('click', '#update_click', function() {
        ...
        var url = "{{ route('manage_template_product.update', ':status') }}";
    ...
    function delete_click(id) {
        ...
        var url = "{{ route('manage_template_product.delete', ':id') }}";
