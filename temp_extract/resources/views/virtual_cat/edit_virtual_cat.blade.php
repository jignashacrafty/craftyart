...
<div class="main-container seo-access-container">
                            ...
                            <select class="selectpicker form-control" data-style="btn-outline-primary"
                                ...
                                @foreach ($datas['app'] as $app)
                                    @if ($datas['cat']->app_id == $app->id)
                                        <option value="{{ $app->id }}" selected="{{ $datas['cat']->app_id }}">
                                            {{ $app->app_name }}
                                        </option>
                                    @else
                    ...
                    @section('content')
                        <div id="virtualcontainer">
                            @include('partials.virtual_section', [
                                'virtualCondition' => htmlentities(
                                    json_encode($datas['virtualCondition'] ?? []),
                                    ENT_QUOTES,
                                    'UTF-8'),
                                'nameset' => true,
                                'limitSet' => false,
    ...
    <script>
        ...
        $('#dynamic_form').on('submit', function(event) {
            ...
            var url = "{{ route('submit_virtual_cat', [$datas['cat']->id]) }}";
